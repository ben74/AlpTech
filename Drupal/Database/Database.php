...
class Database {
    ...
    static $connections = [];
    static $databaseInfo = [];
    static $ignoreTargets = [];
    static $activeKey = 'default';
    static $logs = [];
    static function startLog($logging_key, $key = 'default') {
        if (empty(self::$logs[$key])) {
            self::$logs[$key] = new \Drupal\Core\Database\Log($key);
    ...
    static function getLog($logging_key, $key = 'default') {
        if (empty(self::$logs[$key])) {
            return [];
        }
    ...
    static function getConnection($target = 'default', $key = NULL) {
        $breakpoint=1;
        if (!isset($key)) {
    ...
    static function isActiveConnection() {
        return !empty(self::$activeKey) && !empty(self::$connections) && !empty(self::$connections[self::$activeKey]);
    }

    static function setActiveConnection($key = 'default') {
    ...
    static function parseConnectionInfo(array $info) {
    ...
    static function addConnectionInfo($key, $target, array $info) {
        $a=spl_autoload_functions();#placed above wtf !!!
        if (empty(self::$databaseInfo[$key][$target])) {
            self::$databaseInfo[$key][$target] = self::parseConnectionInfo($info);
        }
    }
    static function getConnectionInfo($key = 'default') {
        if (!empty(self::$databaseInfo[$key])) {
            return self::$databaseInfo[$key];
        }
    }
    static function getAllConnectionInfo() {
        return self::$databaseInfo;
    }
    static function setMultipleConnectionInfo(array $databases) {
    ...
    static function renameConnection($old_key, $new_key) {
    ...
    static function removeConnection($key) {
    ...
    static function openConnection($key, $target) {
/*
...
*/
        ...
        if (!isset(self::$databaseInfo[$key])) {
            $a=1;
            throw new \Drupal\Core\Database\ConnectionNotDefinedException('The specified database connection is not defined: ' . $key);
        }

        if (!$driver = self::$databaseInfo[$key][$target]['driver']) {
            throw new  \Drupal\Core\Database\DriverNotSpecifiedException('Driver not specified for this database connection: ' . $key);
        }
...
#
     ...
     */
    static function closeConnection($target = NULL, $key = NULL) {
     ...
     */
    static function ignoreTarget($key, $target) {
     ...
     */
    static function convertDbUrlToConnectionInfo($url, $root) {
     ...
     *   Exception thrown when a module provided database driver does not exist.
     */
    static function findDriverAutoloadDirectory($namespace, $root) {
     ...
     */
    static function getConnectionInfoAsUrl($key = 'default') {
     ...
     */
    static function getDatabaseDriverNamespace(array $connection_info) {
