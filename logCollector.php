...
/*
usage:
php cli.php getConf logCollectorSecret

logCollectorSecret=hophop
...
curl -sSLk -b "apk=$pk" -d '{"host":"hostName","type":"php500","k":"k","v":"v","severity":9}'
...
if(!$_COOKIE)fun::_die('#'.__line__);
if(!isset($_COOKIE['pk']))fun::_die('#'.__line__);
if($_COOKIE['pk']!=$expectedMd5){print_r($_COOKIE);fun::_die('#'.__line__);}

$output='json';if(isset($_COOKIE['output']))$output=$_COOKIE['output'];


$input=trim(file_get_contents('php://input'),"\n\r \0");#could be regular postdata .. what for cli ?
try{
    $json=@json_decode($input,1 /* ,512,JSON_THROW_ON_ERROR */);
}catch (\JsonException $exception) {
    file_put_contents(__file__.'.json.log',"\n".print_r($exception,1),8);
    echo $exception->getMessage(); // displays "Syntax error"
}
if(!$json){#Control character error, possibly incorrectly encoded[
    $e=json_last_error();$m=json_last_error_msg();$x=substr($input,7,10);$chars=str_split($x);foreach($chars as &$v){$v=ord($v);}unset($v);
#is Ascii char 10 "\n" new line => merding everything !!! #WTF???
#iostat -x -p sda | sed -n '7p'
#clear;d1=`iostat -x -p sda | sed -e 's/[^a-zA-Z0-9 _\.% {2,}]/ /g' | tr '\n' '~'`;json='[{"v":"'$d1'"}]';curl -sSLk -b "a1=$a1;b1=$b1" -d "$json";
#clear;d1=`iostat -x -p sda | sed -e 's/[^a]//g' | tr '\n' '~'`;json='[{"v":"'$d1'"}]';curl -sSLk -b "a1=$a1;b1=$b1" -d "$json";echo '';echo $d1;#pass :)
#clear;d1=`iostat -x -p sda | sed -e 's/[^a]//g'`;json='[{"v":"'$d1'"}]';curl -sSLk -b "a1=$a1;b1=$b1" -d "$json";echo '';echo $d1
    file_put_contents(__file__.'.json.log',"\n".date('Ymd His')."--\n".$x."\n".implode(',',$chars),8);
    echo '#jsonError#'.__line__.'#';
    print_r($e);
    print_r($m);
    fun::_die();
}
...
if(isset($json['sqlSingle'])){
    ...
    if(!$ret)fun::_die('#sql exception#'.__line__);
    #die(json_encode(['results'=>$ret]));
    fun::_die(end(end($ret)));
}
if(isset($json['sql'])){
    $ret=sql($json['sql'],$s['h'],$s['u'],$s['p'],$s['db']);
    foreach($ret as &$t){
        if(in_array(substr($t['v'],0,1),['[','{']) and in_array(substr($t['v'],-1),[']','}']))$t['v']=json_decode($t['v'],1);
    }unset($t);
#preg_replace("~\s+~is",' ',
#preg_replace("~\\\{2,})~is",'',
    if($output=='js'){echo json_encode($ret,JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES);fun::_die();}

    if($output=='dkv2'){$output='print_r';$kv=[];foreach($ret as $t){$kv[][$t['date'].','.$t['k'].','.$t['k2']]=$t['v'];}$ret=$kv;}
    if($output=='kv2'){$output='print_r';$kv=[];foreach($ret as $t){$kv[][$t['k'].','.$t['k2']]=$t['v'];}$ret=$kv;}
    if($output=='dkv'){$output='print_r';$kv=[];foreach($ret as $t){$kv[][$t['date'].','.$t['k']]=$t['v'];}$ret=$kv;}
    if($output=='kv'){$output='print_r';$kv=[];foreach($ret as $t){$kv[$t['k']]=$t['v'];}$ret=$kv;}#fusionne les clés pour un seul enregistrement
    if($output=='print_r'){echo str_replace('~',"\n",print_r($ret,1));fun::_die();}
...
$ak=array_keys($json);
if($ak[0]===0);else $json=[0=>$json];#is one line

#print_r($json);
foreach($json as $idx=>$data){
    $ak=array_keys($data);
    $diff=array_diff($ak,$tablesFields);
    if($diff){$errors[$idx]==__line__;continue;print_r($diff);echo"\n#".__line__;continue;}
    if(strpos($data['k'],'£')){#oui
        ...
        */
        ...
        foreach($k as $i=>$t){
            if($i>6)continue;#ignored
            if(!$i)$data['k']=$t;#0
            else $data['k'.($i+1)]=$t;#1=>2,3,4,5
...
#$_SERVER['HTTP_X_FORWARDED_FOR']
    $data['ip']=_ip($_SERVER['REMOTE_ADDR']);#length:36#2a01:e0a:2d7:fe0:59d5:d01c:3ba9:b6ac
    foreach($data as &$v){
        #if(in_array(gettype($v),['array','object']))$v=print_r($v,1);
        if(in_array(gettype($v),['array','object']))$v=json_encode($v);
    }unset($v);
...
#load avg 11.94 3.37 1.60 13/349 47961
        if($data['k']=='loadavg' and preg_match_all("~[0-9\.]+~",$data['v'],$m) and intval($m[1])>10){#sur la 5ème minute
            alertMail('alert:'.$data['host'].' load avg','load avg '.$data['v']);

        }elseif($data['k']=='dfsda2' and intval($data['v'])>89){#is usage, not free space
            alertMail('alert:'.$data['host'].' disk usage ','disk usage '.$data['v']);
#a1=`date +%y%m%d`;b1=`date +%d%m%y`;curl -sSLk -b "a1=$a1;b1=$b1" -d '[{"type":"metric","k":"freeMem","v":"MemAvailable:   24736 kB"}]'
            ...
            alertMail('alert:'.$data['host'].' free mem ','free mem '.$data['v'].' '.print_r($m,1));

        }elseif($data['k']=='postqueue' and preg_match("~([0-9]+) Kbytes in ([0-9]+) Requests~",$data['v'],$m) and $m[2]>10){
            alertMail('alert:'.$data['host'].' postqueue ','postqueue: '.$data['v']);

        }elseif($data['k']=='cpuidle_avg' and intval($data['v'])<5){
            alertMail('alert:'.$data['host'].' cpuidle_avg ','cpuidle_avg: '.$data['v']);
            ...
            */
            ...
            if($data['v']>50){#echo"#".__line__;
                ...
                $nb=sql("select count(*) as nb,group_concat(v),group_concat(date) from alerts where k='$k' and date >= DATE_SUB(NOW(), INTERVAL 1 HOUR)",$s['h'],$s['u'],$s['p'],$s['db']);$nb=$nb[0];
                if($nb['nb']>5){
                    alertMail('alert:'.$data['host'].' diskio ','diskio: '.$data['v']." ".$nb['nb']." times within 1 hour <pre>".print_r($nb,1));
                }
    ...
    $sql="insert into logs ".insertValues($data);
    $id=sql($sql,$s['h'],$s['u'],$s['p'],$s['db']);
    if(!$id){$errors[$idx]='#'.$sql;}
    else $inserted[$idx]=$id;
}
